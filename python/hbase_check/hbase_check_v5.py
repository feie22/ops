...
if __name__ == '__main__':
    ...
    for cluster_name in NAME_LIST:
        ...
        SUM_VALUE.append((TIME_STRING, cluster_name, r_md5, r_service, r_loadavg, r_mem_use, r_disk_use, r_inode_use, r_bad_disk, r_swap_use, r_bandwidth, r_connect, r_zombie, r_messages, r_dmesg))
    ...
    try:
        ...
        cur.executemany('insert into hbase_check_sum (start_time,cluster_name,md5, service, loadavg, mem_use, disk_use, inode_use, bad_disk, swap_use, bandwidth, connect, zombie, messages, dmesg) values(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)',SUM_VALUE)
        conn.commit()
        cur.close()
        conn.close()
