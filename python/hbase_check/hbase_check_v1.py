...
def check_bandwidth():
    ...
    (s2, r2) = commands.getstatusoutput(cmd_resault)
    if int(r2) > 0:
        print "error:个别服务器网卡带宽制式异常或个别服务器返回异常"
    else:
...
def check_hadoop_jmx():
    print "info:开始检查hadoop jmx指标"
    ip_list =

    os.system("touch /tmp/%s/connection.log" %CLUSTER_NAME)
    for i in ip_list:
        arg = '%{time_total}'
        cmd = "curl -o /dev/null -s -w %s http://%s:50070/dfshealth.jsp --connect-timeout 5" % (arg, i)
        (s, r) = commands.getstatusoutput(cmd)
        if not int(r):
            os.system("echo '%s connection is timeout' >> /tmp/%s/connection.log" % (i, CLUSTER_NAME))
        else:
            dead_nodes_check(i)
            missing_blocks_chcek()
            dfs_used()

    (s1, r1) = commands.getstatusoutput("cat /tmp/%s/connection.log |wc -l " % CLUSTER_NAME)
    if int(r1):
        print "info:所有节点的50070端口连接正常"
        os.system("rm -rf /tmp/%s/connection.log" % CLUSTER_NAME)
    else:
        print "error:个别节点的50070端口连接超时或返回异常"

    (s2, r2) = commands.getstatusoutput("cat /tmp/%s/dead_nodes.log |wc -l " % CLUSTER_NAME)
    if int(r2):
        print "info:所有节点无dead_nodes"
        os.system("rm -rf /tmp/%s/connection.log" % CLUSTER_NAME)
    else:
        print "error:个别节点存在dead_node或返回异常"

    (s3, r3) = commands.getstatusoutput("cat /tmp/%s/MissingBlocks.log |wc -l " % CLUSTER_NAME)
    if int(r3):
        print "info:所有节点无块丢失"
        os.system("rm -rf /tmp/%s/connection.log" % CLUSTER_NAME)
    else:
        print "error:个别节点存在块丢失或返回异常"

    (s4, r4) = commands.getstatusoutput("cat /tmp/%s/dfs_used.log |wc -l " % CLUSTER_NAME)
    if int(r4):
        print "info:所有节点集群空间使用率正常"
        os.system("rm -rf /tmp/%s/connection.log" % CLUSTER_NAME)
    else:
        print "error:个别节点存在集群空间使用率高或返回异常"


def dead_nodes_check(ip):
    print "info:开始检查节点dead_nodes状态"
    url = "http://%s:50070/jmx?qry=Hadoop:service=NameNode,name=NameNodeActivity" % ip
    page = urllib.urlopen(url)
    data = json.load(page)
    dead_nodes_num = data['beans'][0]['DeadNodes'].count('false')
    if dead_nodes_num:
        os.system("echo '%s DeadNodes is %s' >> /tmp/%s/dead_nodes.log" % (i, dead_nodes_num, CLUSTER_NAME))


def block_check(ip):
    print "info:开始检查节点块丢失"
    url = "http://%s:50070/jmx?qry=Hadoop:service=NameNode,name=FSNamesystem" % ip
    page = urllib.urlopen(url)
    data = json.load(page)
    block_missing_num = data['beans'][0]['MissingBlocks']
    if block_missing_num:
        os.system("echo '%s MissingBlocks is %s' >> /tmp/%s/MissingBlocks.log" % (i, block_missing_num, CLUSTER_NAME))

def dfs_used(ip):
    print "info:开始检查空间利用率"
    url = "http://%s:50070/jmx?qry=Hadoop:service=NameNode,name=NameNodeInfo" % ip
    page = urllib.urlopen(url)
    data = json.load(page)
    dfs_used = data['beans'][0]['PercentRemaining']
    if dfs_used > 90:
        os.system("echo '%s DfsUsed is %s' >> /tmp/%s/dfs_used.log" % (i, dfs_used, CLUSTER_NAME))
