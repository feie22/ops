...
def log_output(check_name, error, info):
    ...
    if int(r):
        ...
        ret_tuple = (TIME_STRING, cluster_name, check_name, 'NO',cluster_log_dir + '/' + check_name + '.log')
        VALUE_LIST.append(ret_tuple)
    else:
        os.system('echo "%s: %s INFO:%s" >> %s ' % (cluster_name, CHECK_TIME, info, check_log))
        os.system('rm -rf %s/%s.log' % (cluster_log_dir, check_name))
        ret_tuple = (TIME_STRING, cluster_name, check_name, 'OK','')
        VALUE_LIST.append(ret_tuple)
...
# 基础服务安装检查
def check_service(cluster_name):
...
# 系统日志错误检查
def check_messages(cluster_name):
...
# 系统负载检查
def check_loadavg(cluster_name):
...
# 磁盘空间使用率率检查
def check_disk_use(cluster_name):
...
# 磁盘inode利用率检查
def check_inode_use(cluster_name):
...
def check_mem_use(cluster_name):
...
def check_swap_use(cluster_name):
...
def check_bandwidth(cluster_name):
...
def check_connect(cluster_name):
...
def check_bad_disk(cluster_name):
...
if __name__ == '__main__':
    for cluster_name in NAME_LIST:
        ...
        check_md5()
        check_service(cluster_name)
        check_messages(cluster_name)
        check_dmesg(cluster_name)
        check_loadavg(cluster_name)
        check_disk_use(cluster_name)
        check_inode_use(cluster_name)
        check_mem_use(cluster_name)
        check_swap_use(cluster_name)
        check_bandwidth(cluster_name)
        check_connect(cluster_name)
        check_zombie(cluster_name)
        check_bad_disk(cluster_name)
    ...
    try:
        ...
        cur=conn.cursor()
        cur.executemany('insert into HBASE_CHECK_LOG (start_time,cluster_name,check_itme,check_result,log_location) values(%s,%s,%s,%s,%s)',VALUE_LIST)
        conn.commit()
        cur.close()
        conn.close()
