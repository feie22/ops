...
def check_megacli():
    ...
    cmd = """/opt/MegaRAID/MegaCli/MegaCli64 -Pdlist -aall 2> /dev/null | awk -F ":" '{arry[NR]=$2;if($1=="Media Error Count" && $2>1000){print "Slot Number: "arry[NR-3]" ----- "$0} else if($1=="Other Error Count" && $2>1000){print "Slot Number: "arry[NR-4]" ----- "$0} else if($1=="Last Predictive Failure Event Seq Number" && $2>1000){print "Slot Number: "arry[NR-6]" ----- "$0} else if($1=="Drive has flagged a S.M.A.R.T alert " && $2==" Yes"){print "Slot Number: "arry[NR-40]" ----- "$0}}'"""
    ret_dic = CLIENT.cmd(ip_list, 'cmd.run', [cmd], expr_form='list')
    for i in ip_list:
        if ret_dic.has_key(i):
            if ret_dic[i]:
...
if __name__ == '__main__':
    ...
    NAME_LIST_NO = ['.git', 'README.md', '_public', 'sleep', 'eagle', 'eagle_cattle', 'eagle_major', 'auto-test', 'auto-test_major', 'auto-test_wdl', 'koala', 'sleep_1.1.2', 'ant', 'elephant', 'sleep_0.94', 'vesta', 'vesta_r', 'pan_0.94']
