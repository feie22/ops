...
if len(sys.argv) < 5:
    sys.exit("脚本执行格式：python hbase.py [hbase|dn|rs] [hadoop集群名称] [hbase集群名称] [上线ID]")
T_ID = sys.argv[1]
if T_ID not in ['hbase', 'dn', 'rs']:
    sys.exit("上线类型参数错误[hbase|dn|rs]")
HADOOP_NAME = sys.argv[2]
HBASE_NAME = sys.argv[3]
ID = sys.argv[4]
...
def get_ip():
    ...
    for i in range(0, len(data_list1)):
        ...
        if data_list1[i]['name'] == HBASE_NAME:
            hbase_id = data_list1[i]['id']
    if not hadoop_id:
        sys.exit("集群%s id获取失败，BDP中无此集群" % HADOOP_NAME)
    elif not hbase_id:
        sys.exit("集群%s id获取失败，BDP中无此集群" % HBASE_NAME)
...
def set_hbase_conf(dns_dic, hbase_rack_dic, version_dic, hbase_ip_dic):
    ...
    if hbase_ip_dic['HM']:
        ...
        hbase_site = GIT_DIR + HBASE_NAME + '/software/servers/' + version_dic['hbase_v'] + '/conf/hbase-site.xml'
        cmd = "sed -ir 's/\/hbase.*</\/hbase_%s</g' %s" % (HBASE_NAME, hbase_site)
        (s, r) = commands.getstatusoutput(cmd)
        if s:
            sys.exit("修改hbase-site.xml文件失败")
...
def sync_ssh(hbase_ip_dic):
    ...
    ret = saltapi.main_async(params)
    jid = ret[0]['jid']
    content = "sync_authorized_keys jid：" + jid + '\n'
    write_file(LOG_FILE, content)
    time.sleep(10)
    fun = 'cmd.run'
    arg = "chown -R hadp:hadp /home/hadp/.ssh ; chmod 700 /home/hadp/.ssh; chmod 600 /home/hadp/.ssh/*"
    params = create_params(fun, hbase_ip_dic['ALL'], arg)
    ret = saltapi.main_async(params)
    jid = ret[0]['jid']
    content = "chown_ssh_keys jid：" + jid + '\n'
    write_file(LOG_FILE, content)

    content = "ssh-key异步同步完成\n"
    write_file(LOG_FILE, content)
    # ret_dick = ret[0]

def sync_hbase_conf(hbase_ip_dic, version_dic):
    fun1 = "cp.get_dir"
    arg1 = "salt://%s/software/servers/%s" % (HBASE_NAME, version_dic['hbase_v'])
    arg2 = "/software/servers"
    params = create_params(fun1, hbase_ip_dic['ALL'], arg1, arg2)
    ret = saltapi.main_async(params)
    jid = ret[0]['jid']
    content = "sync_hbase_sofware jid：" + jid + '\n'
    write_file(LOG_FILE, content)
    time.sleep(len(hbase_ip_dic['ALL']) + 10)
    content = "hbase配置异步同步完成\n"
    write_file(LOG_FILE, content)
...
def sync_hadoop_conf(hadoop_ip_dic):
    ...
    write_file(LOG_FILE, content)
    time.sleep(len(hadoop_ip_dic['NO_ZK']) + 10)
...
# def start_service(hbase_ip_dic):
...
if __name__ == '__main__':
    ...
    exec_dic = {'hbase':[1, 2, 3, 4, 5, 6, 7, 8],
                'dn':[1, 3, 5, 6, 8],
                'rs':[2, 4, 5, 6, 7]
                }

    for i in exec_dic[T_ID]:
        if i == 1:
            ...
            sync_hbase_conf(hbase_ip_dic, version_dic)
        elif i == 8:
