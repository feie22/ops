...
class RabitTracker(object):
    ...
    def accept_slaves(self, nslave):
        ...
        while len(shutdown) != nslave:
            ...
            if s.cmd == 'print':
                msg = s.sock.recvstr()
                msgLower = msg.strip().lower()
                #logging.info(msg.strip())
                #if xgboost has error:
                #    sock.shudown
                #    sock.close
                #cli_main.cc --> JD-XGB-ERROR:
                if msgLower.find("jd-xgb-error:") != -1:
                    logging.info(msg.strip())
                    self.sock.shutdown(socket.SHUT_RDWR)
                    self.sock.close()
                    shutdown={}
                    for tmp in range(0,nslave):
                        shutdown[tmp] = None
                elif msgLower.find("trainperc") != -1:
                    arr1 = msgLower.split("#")
                    rank = -1
                    perc = 0.0
                    percTotal = 0.0
                    for s1 in arr1:
                        arr2 = s1.split("=")
                        if (arr2[0]).find("rank") != -1:
                            rank = int(arr2[1])
                        elif (arr2[0]).find("perc") != -1:
                            perc = float(arr2[1])
                    if rank > -1 and perc > 0.0:
                        perc_map[rank] = perc
                        for v in perc_map.values():
                            percTotal = percTotal + v
                        #logging.info(str(1.0 * percTotal / total))
                        self.update_progress(round(1.0 * percTotal / total,4))
                else:
                    logging.info(msg.strip())
                continue
            if s.cmd == 'shutdown':
            ...
            if rank == -1:
                ...
                if len(todo_nodes) == 0:
                    ...
                    logging.info('@tracker loading data ... ...')
    ...
    def join(self):
        while self.thread.isAlive():
            if END == 0:
                ...
                logging.warn("tracker exit with an exception")
                break
