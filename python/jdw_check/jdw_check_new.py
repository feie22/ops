    ...
    for conf, md5 in pub_md5_dic.iteritems():
        ...
        for i in ip_alive:
            if ret_dic.has_key(i):
                ...
                elif len(ret_dic[i]) != 32:
                    print "%s == %s == %s" % (i, conf, cluster_log_dir )
            else:
...
def check_megacli():
    ...
    cmd = """/opt/MegaRAID/MegaCli/MegaCli64 -Pdlist -aall 2> /dev/null | awk -F ":" '{arry[NR]=$0;if($1=="Media Error Count" && $2>1000){print arry[NR-5]"-----"$0} else if($1=="Other Error Count" && $2>1000){print arry[NR-6]"-----"$0} else if($1=="Last Predictive Failure Event Seq Number" && $2>1000){print arry[NR-8]"-----"$0} else if($1=="Drive has flagged a S.M.A.R.T alert " && $2==" Yes"){print arry[NR-42]"-----"$0}}'"""
    ret_dic = CLIENT.cmd(ip_alive, 'cmd.run', [cmd], expr_form='list')
    for i in ip_alive:
        if ret_dic.has_key(i):
            if ret_dic[i]:
