...
def get_ip_alive(p_name):
    ...
    ip_offline = []
    for t_id in t_jkzt:
        url_jkzt = 'http://bdp.jd.com/ops/api/server/searchIpsByTags.ajax?tags=%s,%s&appKey=123456&erp=wangdelong5' % (p_name,t_id)
...
def log_output(check_name, error, info):
    (s, r) = commands.getstatusoutput("grep -E -o '([0-9]{1,3}\.){1,3}[0-9]{1,3}' %s/%s.log| wc -l" % (p_log_dir, check_name))
    if int(r):
...
if __name__ == '__main__':
    ...
    #send_phenix(p_name, 'mega errors', r_mega_errors, 'i')
