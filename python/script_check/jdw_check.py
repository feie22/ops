...
def get_ip(CLUSTER_NAME):
    ...
    for i, n in tag_dic.iteritems():
        ...
        #ip_dic[i] = ip_list
        ip_dic[i] = list(set(ip_list).difference(set(ip_offline)))
    ip_all_list = []
    # print ip_dic.keys()
    for i in tag_list:
    ...
    os.system('> %s/alive.log' % LOG_DIR)
    ret_dic = {}
    ret_dic1 = CLIENT.cmd_batch(ip_all_list, 'test.ping', expr_form='list', batch='20%')
    for j in ret_dic1:
...
def check_md5():
    ...
    for conf, md5 in pub_md5_dic.iteritems():
        ...
        ret_dic = {}
        ret_dic1 = CLIENT.cmd_batch(ip_alive, 'cmd.run', [cmd], expr_form='list', batch='20%')
        for j in ret_dic1:
            ret_dic = dict(ret_dic, **j)
        for i in ip_alive:
            if ret_dic.has_key(i):
                if ret_dic[i] != md5 or len(ret_dic[i]) != 32:
                ...
                os.system("echo '%s 程序中md5 public 这个模块异常' >> %s/md5.log" % (i, cluster_log_dir ))
    # 判断内存及yarn-site
    mem_cmd = "md5sum /software/servers/hadoop-2.2.0/etc/hadoop/yarn-site.xml | awk '{print $1}' "
    mem_ret_dic = {}
    mem_ret_dic1 = CLIENT.cmd_batch(ip_alive, ['grains.item','cmd.run'], [['mem_total'],[mem_cmd]], expr_form='list', batch='20%')
    for j in mem_ret_dic1:
        mem_ret_dic = dict(mem_ret_dic, **j)
    for i in ip_alive:
        if mem_ret_dic.has_key(i):
            ...
            #os.system("echo '%s salt-minon can not be connected!' >> %s/md5.log" % (i, cluster_log_dir))
            os.system("echo '%s 程序中md5sum yarn-site.xml这个模块异常' >> %s/md5.log" % (i, cluster_log_dir))
    # print "判断public配置完成"
    # 判断dn节点
    for conf, md5 in dn_md5_dic.iteritems():
        ...
        for i in ip_dic['dn']:
            if ret_dic.has_key(i):
                ...
                #os.system("echo '%s salt-minon can not be connected!' >> %s/md5.log" % (i, cluster_log_dir))
                os.system("echo '%s 程序中md5sum dn这个模块异常' >> %s/md5.log" % (i, cluster_log_dir))
    # print "判断dn配置完成"
    # 判断ns1-ns5
    for ns in ns_tuple:
        ip_list = ip_dic[ns]
        for conf, md5 in ns_md5_dic[ns].iteritems():
            cmd = "md5sum " + conf + " | awk '{print $1}' "
            ret_dic = {}
            ret_dic1 = CLIENT.cmd_batch(ip_list, 'cmd.run', [cmd], expr_form='list', batch='20%')
            for j in ret_dic1:
                ret_dic = dict(ret_dic, **j)
            for i in ip_list:
                if ret_dic.has_key(i):
                    ...
                    #os.system("echo '%s salt-minon can not be connected!' >> %s/md5.log" % (i, cluster_log_dir))
                    os.system("echo '%s 程序中md5sum ns 这个模块异常' >> %s/md5.log" % (i, cluster_log_dir))
    # print "判断ns1-ns5配置完成"
...
# 基础服务安装检查
def check_service():
    os.system('> %s/service.log' % LOG_DIR)
...
# 磁盘空间使用率率检查
def check_disk_use():
    os.system('> %s/disk_use.log' % LOG_DIR)
...
def check_bandwidth():
    os.system('> %s/bandwidth.log' % LOG_DIR)
...
def check_connect():
    os.system('> %s/netstat.log' % LOG_DIR)
...
def check_zombie():
    os.system('> %s/zombie.log' % LOG_DIR)
...
def check_megacli():
    os.system('> %s/mega_errors.log' % LOG_DIR)
...
def check_net_errors():
    os.system('> %s/net_errors.log' % LOG_DIR)
...
def check_bad_disk():
    ...
    os.system('> %s/bad_disk.log' % LOG_DIR)
    ...
    os.system('> %s/miss_disk.log' % LOG_DIR)
